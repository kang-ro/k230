...
class FaceDetectionApp(AIBase):
    ...
    def draw_result(self, pl, dets, elapsed_time=0):
        with ScopedTiming("display_draw", self.debug_mode > 0):
            ...
            # 在屏幕右上角显示系统信息面板 / Display system info panel at top right corner
            self.draw_info_panel(pl, elapsed_time)

    def draw_info_panel(self, pl, elapsed_time):
        ...
        total_mem = heap_info[0]
        free_mem = heap_info[1]
        used_mem = heap_info[2]
        ...
        time_str = "时间: {:02d}:{:02d}:{:02d}".format(hours, minutes, seconds)
        
        # 格式化内存使用信息 / Format memory usage information
        mem_str = "内存: {:.1f}%".format(mem_usage_percent)
        
        # 设置字体大小和颜色 / Set font size and color
        font_size = 4
        text_color = (255, 255, 255, 255)  # 白色 / White color
        
        # 构建信息文本列表 / Build information text list
        info_lines = [time_str, mem_str, "温度: " + temp_str]
        
        # 计算信息面板尺寸 / Calculate info panel dimensions
        max_line_length = max(len(line) for line in info_lines)
        line_height = font_size * 12
        panel_width = max_line_length * font_size * 6 + 40
        panel_height = len(info_lines) * line_height + 20
        
        # 计算面板位置（屏幕右上角） / Calculate panel position (top right corner)
        margin = 20
        panel_x = self.display_size[0] - panel_width - margin
        panel_y = margin
        
        # 绘制半透明背景框 / Draw semi-transparent background box
        pl.osd_img.draw_rectangle(panel_x, panel_y, panel_width, panel_height, 
                                color=(0, 0, 0, 160), thickness=-1)
        
        # 绘制边框 / Draw border
        pl.osd_img.draw_rectangle(panel_x, panel_y, panel_width, panel_height, 
                                color=(255, 255, 255, 255), thickness=2)
        
        # 绘制信息文本 / Draw information text
        text_x = panel_x + 10
        text_y = panel_y + 10
        
        for i, line in enumerate(info_lines):
            pl.osd_img.draw_string(text_x, text_y + i * line_height, line, 
                                 scale=font_size, color=text_color, thickness=2, fill=True)
...
def exce_demo(pl):
    ...
    # 记录程序开始时间 / Record program start time
    ...
    try:
        ...
        while True:
            with ScopedTiming("total",0):    # 计时器 / Timer
                ...
                # 计算程序运行时间 / Calculate program running time
                current_time = utime.ticks_ms()
                elapsed_time = utime.ticks_diff(current_time, start_time) // 1000  # 转换为秒 / Convert to seconds
                
                face_det.draw_result(pl, res, elapsed_time) # 绘制检测结果和计时器 / Draw detection results and timer
