...
class FaceMesh:
    def __init__(self,face_det_kmodel,face_mesh_kmodel,mesh_post_kmodel,det_input_size,mesh_input_size,anchors,confidence_threshold=0.25,nms_threshold=0.3,rgb888p_size=[640,360],display_size=[640,360],debug_mode=0):
...
def exce_demo(pl):
    ...
    fm=FaceMesh(face_det_kmodel_path,face_mesh_kmodel_path,face_mesh_post_kmodel_path,
    ...
    try:
        while True:
            with ScopedTiming("total",0):
                img=pl.get_frame()                      # 获取视频帧 / Get video frame
                det_boxes,mesh_res=fm.run(img)          # 执行推理 / Perform inference
        ...
        print("人脸网格功能退出")                       # 打印退出信息 / Print exit message
    finally:
