...
def load_image():
    # 尝试打开并读取PNG图片文件
    # Try to open and read the PNG image file
    try:
        with open("/sdcard/resources/wp.png",'rb') as f:
            png_data = f.read()
    except:
        print("Could not find logo") # 无法找到logo文件 / Could not find logo file

    # 创建LVGL图像描述符
    # Create LVGL image descriptor
    img_logo = lv.img_dsc_t({
      'data_size': len(png_data),  # 设置图像数据大小 / Set image data size
      'data': png_data             # 设置图像二进制数据 / Set image binary data
    })

    # 创建图像对象并显示在屏幕上
    # Create image object and display on screen
    img1 = lv.img(lv.scr_act())    # 创建图像控件 / Create image widget
    img1.set_src(img_logo)         # 设置图像源 / Set image source
    img1.align(lv.ALIGN.CENTER, 0, 0)  # 居中对齐 / Center alignment
